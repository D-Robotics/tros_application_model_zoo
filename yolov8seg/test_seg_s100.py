...
def gettensor(file, h, w, scale_data, dtype=np.int32):

    vec = np.fromfile(file, dtype=dtype).astype(np.float32)

    vec = np.split(vec, h * w)

    output = np.array([seg * scale_data for seg in vec]).reshape(1, h, w, -1)

    return output

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--model-path', type=str, default='../yolov8n_seg_output_s100/yolov8n_seg_640x640_nv12_quantized_model.bc', help='Path to Horizon BPU Quantized *.bin Model.\nxj3 (bernoulli2) or xj5 (bayes)')
...
class YOLOv8_Seg_img():
    def __init__(self, opt):
        ...
        self.sess = HBRuntime(opt.model_path)
    ...
    def forward(self, input_img):
        input_names = self.sess.input_names
        output_names = self.sess.output_names
        feed_dict = dict()
        inputs = self.preprocess(input_img, is_processed=True)
        for i in range(len(input_names)):
            feed_dict[input_names[i]] = inputs[i]
    ...
    def preprocess(self, image_name, is_processed):
        ...
        processed_image_y = processed_image[0][: 409600]
        processed_image_uv = processed_image[0][409600: ]
        processed_image_y = processed_image_y.reshape(1, 640, 640, 1)
        processed_image_uv = processed_image_uv.reshape(1, 320, 320, 2)
        
        return [processed_image_y, processed_image_uv] if is_processed else origin_image
